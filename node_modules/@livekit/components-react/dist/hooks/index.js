...
function useMediaTrackBySourceOrName(observerOptions, options = {}) {
  var _a;
  const [publication, setPublication] = React23.useState((0, import_components_core19.getTrackByIdentifier)(observerOptions));
...
function useParticipantInfo(props = {}) {
  ...
  const infoObserver = React26.useMemo(() => (0, import_components_core21.participantInfoObserver)(p), [p]);
...
function useParticipantPermissions(options = {}) {
  const p = useEnsureParticipant(options.participant);
  const permissionObserver = React27.useMemo(() => (0, import_components_core22.participantPermissionObserver)(p), [p]);
...
function useRoomInfo(options = {}) {
  const room = useEnsureRoom(options.room);
  const infoObserver = React32.useMemo(() => (0, import_components_core26.roomInfoObserver)(room), [room]);
...
function useSpeakingParticipants() {
  const room = useRoomContext();
  const speakerObserver = React33.useMemo(() => (0, import_components_core27.activeSpeakerObserver)(room), [room]);
...
function useChatToggle({ props }) {
  ...
  const { className } = React38.useMemo(() => (0, import_components_core31.setupChatToggle)(), []);
...
function useIsEncrypted(participant) {
  ...
  const observer = React45.useMemo(() => (0, import_components_core38.encryptionStatusObservable)(room, p), [room, p]);
